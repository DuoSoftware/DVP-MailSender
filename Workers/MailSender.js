...
var CreateEngagement = require("dvp-common/ServiceAccess/common")
  .CreateEngagement;
...
function createEngagement(engagementObj) {
  ...
  CreateEngagement(
    "email",
    engagementObj.company,
    engagementObj.tenant,
    engagementObj.from_email,
    engagementObj.to_email,
    "outbound",
    engagementObj.messageId,
    engagementObj.subject,
    function(done) {
      if (done) {
...
function flushWaitingMessages() {
  var send = function(data) {
    ...
    console.log("-------- Step 1 -------------");
    Org.findOne(
      { tenant: data.message.tenant, id: data.message.company },
      function(err, org) {
        console.log("-------- Step 2 -------------");
        if (err) {
            ...
            Email.findOne(
              ...
              function(err, email) {
                ...
                if (err) {
                  ...
                  mailSender
                    .mailSenderFactory(emailSendMethod)
                    ...
                    .then(function(info) {
                      ...
                      if (info.mailDetails.engagement) {
                        ...
                        console.log(
                          "engagement creation is not specified in the email object but in email config"
                        );
                        createEngagement(info.mailDetails);
                      } else {
